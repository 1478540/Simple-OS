...
static void intr_keyboard_handler(void)
{
    ...
    else if((scancode > 0x00 && scancode < 0x3b) || (scancode == alt_r_make) || (scancode == ctrl_r_make)||(scancode==up_make)||(scancode==down_make)||(scancode==left_make)||(scancode==right_make))
    {
	...
	if ((scancode < 0x0e) || (scancode == 0x29) || (scancode == 0x1a) || \
        ...
        // #define right_make 0xe04d
        char cur_char ;
        if(index==0x48||index==0x50||index==0x4b||index==0x4d){
            if(index==0x48){
                cur_char=keymap2[0][shift];
            }else if(index==50){
                cur_char=keymap2[1][shift];
            }else if(index==0x4b){
                cur_char=keymap2[2][shift];
            }else if(0x4d==index){
                cur_char=keymap2[3][shift];
            }
        }
        else cur_char = keymap[index][shift];
        if (cur_char) {
            /*****************  快捷键ctrl+l和ctrl+u的处理 *********************
